...
build-mfa: build-prerequisites
	go build -ldflags "-X main.version=$${VERSION:-0.0.0} -X main.commit=$$(git rev-parse --short HEAD 2>/dev/null || echo \"none\")" -o bin/$(OUTPUT_DIR)mfa cli/main.go
...
archive-mfa-linux_amd64: build-mfa-linux_amd64
	tar czf dist/mfa-$ -0.0.0-linux_amd64.tar.gz -C bin/linux_amd64/ .
archive-mfa-darwin_amd64: build-mfa-darwin_amd64
	tar czf dist/mfa-$ -0.0.0-darwin_amd64.tar.gz -C bin/darwin_amd64/ .
archive-mfa-windows_amd64: build-mfa-windows_amd64
	tar czf dist/mfa-$ -0.0.0-windows_amd64.tar.gz -C bin/windows_amd64/ .
...
release: archive
	sha1sum dist/*.tar.gz > dist/mfa-hB0.0.0.shasums
